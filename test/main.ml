...
let database_add_table_tests =
  [
    (* database_add_table_test "table1" "table1" [ ("attr1", Int 1);
       ("attr2", Bool false); ("attr3", Float 0.1) ] "db1"; *)
...
let database_drop_table_tests =
  [
    (let _ = Mem.add_table "table1" [ ("hi", Int 1) ] "db2" () in
     database_drop_table_test "drop" "table1" "db2");
  ]
