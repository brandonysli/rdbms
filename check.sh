...
ZIP=final.zip
ZIPCHECKFILES=(src/database.ml src/mem.ml test/main.ml testparse/main.ml bin/repl.ml)
...
for i in "${ZIPCHECKFILES[@]}"; do
  if ! zipinfo -1 "$ZIP" | grep -q "^$i$"; then
